...
void InclineUpdateAgent::update() {
    ...
    if ((x < (DEGREES_CIRCLE - INCLINE_THRESHOLD) && x > INCLINE_THRESHOLD) ||
        (y < (DEGREES_CIRCLE - INCLINE_THRESHOLD) && y > INCLINE_THRESHOLD)) {
        // The threshold is exceeded.
        if (DEBUG && INCLINE_UPDATE_AGENT_DEBUG) {
            Serial.println("INCLINE DETECTED");
        }
        ...
        if (this->state->incline == UNSEEN &&
            ...
            if (DEBUG && INCLINE_UPDATE_AGENT_DEBUG) {
                Serial.println("SETTING INCLINE TO CURRENT");
            }
            this->state->incline = CURRENT;
        }
    } else {
        if (DEBUG && INCLINE_UPDATE_AGENT_DEBUG) {
            Serial.println("NO INCLINE DETECTED");
        }
        // The threshold is not/no longer exceeded.
        if (this->state->incline == CURRENT) {
            if (DEBUG && INCLINE_UPDATE_AGENT_DEBUG) {
                Serial.println("SETTING INCLINE TO COMPLETED");
            }
