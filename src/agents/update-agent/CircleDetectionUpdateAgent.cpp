...
CircleDetectionUpdateAgent::CircleDetectionUpdateAgent(
    State *state, IRSensor *irNW, IRSensor *irNE, IRSensor *irSW,
    IRSensor *irSE, LineFollowerSensor *lf)
    : UpdateAgent(state), lf(lf), irNW(irNW), irNE(irNE), irSW(irSW),
      irSE(irSE) {}
...
void CircleDetectionUpdateAgent::update() {
    ...
    // if we have not completed the line following, then we cannot be close to
    // the circle
    if (this->state->lineFollowingTable != COMPLETED ||
        this->state->robotState == DISARMED) {
        if (DEBUG && CIRCLE_DETECTION_UPDATE_AGENT_DEBUG) {
            Serial.println(
                "LINE FOLLOWING NOT COMPLETED OR ROBOT DISARMED, RETURNING");
        }
        return;
    }
    ...
    // if there is an edge on the left, the IR sensors on the left are not
    ...
    // if every sensor detects a black surface, we are in the circle and have
    // therefore the final table
    if (nw && n && ne && sw && se) {
        // TODO: start timer here and stop after some time passed (stopping in
        // the middle of the table maybe make timer for time on circle (drive
        // forward) and after leaving the circle reverse for half the time?

        // this means we finished the last table, so we are done
        if (DEBUG && CIRCLE_DETECTION_UPDATE_AGENT_DEBUG) {
            Serial.println("IN CIRLCE :)");
        }
