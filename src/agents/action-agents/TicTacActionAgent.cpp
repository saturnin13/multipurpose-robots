...
void TicTacActionAgent::enact() {
    ...
    if(!this->stepperMotor->workInProgress) {
        ...
        if(numberDrops > 0 && (now - this->timeLastDropEnd) > TIME_BETWEEN_DROPS) {

            //first drop has more step (making sure the tictac is acatually dropped)
            int numberSteps = this->numberTicTacs == 4 ?  NUMBER_STEPS_FIRST_TICTAC : NUMBER_STEPS_PER_TICTAC;
        ...
        } else if (this->numberDrops == 0 && (now - this->timeLastDropEnd) > TIME_BETWEEN_DROPS) {
